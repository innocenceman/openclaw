...
- Primary paths: `src/gateway/protocol/**`
- Runtime handoff: Protocol types define the contract shared by gateway runtime, clients, and app/control surfaces.
<!-- version-diff-refresh:v2026.4.24:start -->

## v2026.4.24 target-existing changed source paths

This target refresh maps `29` changed path(s) to this final leaf. `29` path(s) still exist in the target source and are listed below. Deleted or renamed-away paths remain only in version-diff artifacts, not active impact-map. Status counts: `{'M': 27, 'A': 2}`.
