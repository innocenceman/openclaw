...
- Primary paths: `src/media/**`
- Runtime handoff: Media inputs are normalized and stored before channels or agent tools consume them.
<!-- version-diff-refresh:v2026.4.24:start -->

## v2026.4.24 target-existing changed source paths

This target refresh maps `101` changed path(s) to this final leaf. `101` path(s) still exist in the target source and are listed below. Deleted or renamed-away paths remain only in version-diff artifacts, not active impact-map. Status counts: `{'A': 68, 'M': 33}`.
