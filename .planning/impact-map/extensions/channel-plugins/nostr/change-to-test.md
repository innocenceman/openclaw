...
| Runtime dependency/export changed                | `pnpm test -- test/scripts/stage-bundled-plugin-runtime-deps.test.ts` and targeted Nostr tests                                                                   | `pnpm build` is required for package exports, `nostr-tools` boundary changes, or production runtime behavior.          |
