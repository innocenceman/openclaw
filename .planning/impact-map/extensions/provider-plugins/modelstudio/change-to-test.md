# modelstudio Provider Plugin Change-to-Test Matrix
...
| Provider runtime behavior changed       | `pnpm test -- extensions/modelstudio` or nearest provider test          | `pnpm test:extensions` when shared provider helpers are touched.            |
...
- Changed paths: `10` (删除 9，重命名/移动 1).
- Target-existing path refs in active map: `0`; deleted/renamed-away refs kept only in transition artifacts: `10`.
