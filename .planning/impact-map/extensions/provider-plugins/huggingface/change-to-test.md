# huggingface Provider Plugin Change-to-Test Matrix
...
| Provider runtime behavior changed       | `pnpm test -- extensions/huggingface` or nearest provider test          | `pnpm test:extensions` when shared provider helpers are touched.            |
...
- Changed paths: `9` (新增 2，修改 5，重命名/移动 1，删除 1).
- Target-existing path refs in active map: `8`; deleted/renamed-away refs kept only in transition artifacts: `1`.
