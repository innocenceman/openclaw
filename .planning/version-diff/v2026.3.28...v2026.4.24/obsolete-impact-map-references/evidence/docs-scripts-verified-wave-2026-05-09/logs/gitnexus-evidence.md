...
- `context(collectBundledPluginMetadata, scripts/generate-bundled-plugin-metadata.mjs)`: incoming tests in `src/plugins/bundled-plugin-metadata.test.ts`; outgoing calls include bundled source collection and manifest normalization helpers.
...
| `collectBundledPluginMetadata`    | `scripts/generate-bundled-plugin-metadata.mjs`  | `writeBundledPluginMetadataModule`; `src/plugins/bundled-plugin-metadata.test.ts`        |
