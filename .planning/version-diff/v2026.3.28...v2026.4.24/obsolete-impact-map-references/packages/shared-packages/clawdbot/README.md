...
Compatibility shim package for `packages/clawdbot/**`. It forwards package imports to `openclaw` and declares the legacy `clawdbot` binary surface in `packages/clawdbot/package.json`.
...
- Inspected `packages/clawdbot/package.json`, `packages/clawdbot/index.js`, and `packages/clawdbot/scripts/postinstall.js`.
...
Reason: import shim and pack dry-run passed, but `packages/clawdbot/package.json` declares a legacy CLI `bin`/`./cli-entry` target under `./bin/` and that file is absent. Do not mark this package shim `verified` until the declared CLI target exists and the standard release/pack-install ladder passes.
