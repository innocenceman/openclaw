...
translator protocol 所属的 Agent loop, ACP, spawning, command control 有 13 个路径变化（新增 2、修改 11），兼容性判断为行为变化。
...
- 首选验证：1. Run `pnpm test -- src/acp/translator*.test.ts src/acp/event-mapper.test.ts src/acp/client.test.ts src/acp/server.startup.test.ts`.
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `src/capability-modules/acp/translator-protocol` 的 mapping。
