...
device pair 所属的 Model/search/provider plugins and tool surfaces 有 12 个路径变化（新增 4、修改 8），兼容性判断为行为变化。
...
- Status counts: `新增 4、修改 8`
- Risk: `high`
- Compatibility: `behavior-change` (行为变化)
- Diff stat: `12` files, `+1454` / `-355`

## 功能变化摘要

这个 leaf 属于 `Model/search/provider plugins and tool surfaces`。本次版本差异显示它的主要变化是：新增 4、修改 8。以下判断基于路径状态、leaf 所属模块和 active impact-map 说明生成，仍以 `leaf-impact.json` 与源码 diff 为事实来源。
...
- 首选验证：Plugin pairing flow changed | `pnpm test -- extensions/device-pair/index.test.ts` | Add broader gateway/control-ui tests when pairing authz or browser/gateway flows change.
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `extensions/auth-integration-plugins/device-pair` 的 mapping。
