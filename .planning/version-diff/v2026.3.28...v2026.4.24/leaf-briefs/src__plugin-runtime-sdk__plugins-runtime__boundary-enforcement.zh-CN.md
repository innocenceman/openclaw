...
boundary enforcement 所属的 Plugin SDK, plugin registry, plugin loading boundaries 有 2 个路径变化（新增 1、修改 1），兼容性判断为行为变化。
...
- Diff stat: `2` files, `+216` / `-28`

## 功能变化摘要

这个 leaf 属于 `Plugin SDK, plugin registry, plugin loading boundaries`。本次版本差异显示它的主要变化是：新增 1、修改 1。以下判断基于路径状态、leaf 所属模块和 active impact-map 说明生成，仍以 `leaf-impact.json` 与源码 diff 为事实来源。
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `src/plugin-runtime-sdk/plugins-runtime/boundary-enforcement` 的 mapping。
