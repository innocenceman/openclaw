...
mobile native settings 所属的 Native apps and packaging 有 8 个路径变化（新增 3、修改 5），兼容性判断为行为变化。
...
- Diff stat: `8` files, `+591` / `-5`

## 功能变化摘要

这个 leaf 属于 `Native apps and packaging`。本次版本差异显示它的主要变化是：新增 3、修改 5。以下判断基于路径状态、leaf 所属模块和 active impact-map 说明生成，仍以 `leaf-impact.json` 与源码 diff 为事实来源。
...
- 首选验证：1. Run Android settings/unit coverage with `pnpm android:test` and `pnpm android:assemble` when Android settings change.
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `apps/provider-channel-settings/native-settings/mobile-native-settings` 的 mapping。
