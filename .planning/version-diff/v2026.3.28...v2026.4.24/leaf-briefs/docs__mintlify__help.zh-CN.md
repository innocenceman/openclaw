...
help 所属的 Docs, generated baselines, changelog and public guidance 有 12 个路径变化（新增 5、修改 7），兼容性判断为仅文档/测试。
...
- Status counts: `新增 5、修改 7`
- Risk: `medium`
- Compatibility: `docs-only` (仅文档/测试)
- Diff stat: `12` files, `+3419` / `-1683`

## 功能变化摘要

这个 leaf 属于 `Docs, generated baselines, changelog and public guidance`。本次版本差异显示它的主要变化是：新增 5、修改 7。以下判断基于路径状态、leaf 所属模块和 active impact-map 说明生成，仍以 `leaf-impact.json` 与源码 diff 为事实来源。
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `docs/mintlify/help` 的 mapping。
