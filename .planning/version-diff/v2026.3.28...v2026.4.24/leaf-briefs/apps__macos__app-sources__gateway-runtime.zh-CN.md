...
gateway runtime 所属的 Native apps and packaging 有 10 个路径变化（新增 1、修改 9），兼容性判断为行为变化。
...
- Status counts: `新增 1、修改 9`
- Risk: `high`
- Compatibility: `behavior-change` (行为变化)
- Diff stat: `10` files, `+424` / `-86`

## 功能变化摘要

这个 leaf 属于 `Native apps and packaging`。本次版本差异显示它的主要变化是：新增 1、修改 9。以下判断基于路径状态、leaf 所属模块和 active impact-map 说明生成，仍以 `leaf-impact.json` 与源码 diff 为事实来源。
...
- 首选验证：4. Run `pnpm check:host-env-policy:swift` when generated host env policy or launch/runtime environment handling is in scope.
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `apps/macos/app-sources/gateway-runtime` 的 mapping。
