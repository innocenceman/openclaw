...
diagnostics otel 所属的 Model/search/provider plugins and tool surfaces 有 4 个路径变化（新增 1、修改 3），兼容性判断为行为变化。
...
- 首选验证：Exporter/service behavior changed | `pnpm test -- extensions/diagnostics-otel/src/service.test.ts` | Add broader telemetry/runtime validation if startup, sampling, or event mapping semantics change.
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `extensions/auth-integration-plugins/diagnostics-otel` 的 mapping。
