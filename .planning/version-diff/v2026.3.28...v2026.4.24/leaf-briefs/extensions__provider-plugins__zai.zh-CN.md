...
zai 所属的 Model/search/provider plugins and tool surfaces 有 15 个路径变化（新增 4、修改 9、删除 2），兼容性判断为行为变化。
...
- Status counts: `新增 4、修改 9、删除 2`
- Risk: `high`
- Compatibility: `behavior-change` (行为变化)
- Diff stat: `15` files, `+357` / `-59`

## 功能变化摘要

这个 leaf 属于 `Model/search/provider plugins and tool surfaces`。本次版本差异显示它的主要变化是：新增 4、修改 9、删除 2。以下判断基于路径状态、leaf 所属模块和 active impact-map 说明生成，仍以 `leaf-impact.json` 与源码 diff 为事实来源。
...
- `.planning/version-diff/v2026.3.28...v2026.4.24/leaf-impact.json` 中 leaf `extensions/provider-plugins/zai` 的 mapping。
